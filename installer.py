...
REQUIREMENTS = ["tqdm", "colorama", "pyfiglet", "prettytable", "pytest-cov"]
...
def main():
    ...
    if apt_available:
        ...
        print(
            "APT not found. Only Python Virtual Environment (recommended) "
            "is available."
        )
        choice = "1"
    ...
    if choice == "1":
        pref_path = input(
            "Enter preferred directory for venv (leave blank for ~/.venv): "
        ).strip()
        if pref_path:
        ...
        auto_create = (
            input(
                f"Do you want to automatically create the venv and install "
                f"dependencies at {venv_path}? (y/n): "
            )
            .strip()
            .lower()
        )
        if auto_create == "y":
            ...
            print(
                f"\nSkipping automatic creation. You can create it later "
                f"using: python3 -m venv {venv_path}"
            )
            print_venv_guide(venv_path)
