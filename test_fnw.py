...
def test_collect_scan_choices_single_tcp_udp(monkeypatch):
    ...
    user_inputs = _inputs(
        "2", "1", "y",  # TCP -> internal, yes add more
        "3", "2", "n",  # UDP -> external, no more
        "4"             # Done
    )
...
def test_collect_scan_choices_prevent_multiple_discovery(monkeypatch, capsys):
    ...
    user_inputs = _inputs(
        "1", "192.168.0.0/30", "y",  # Discovery, yes add more
        "2", "1", "n",               # TCP -> internal, no more
        "4"                           # Done
    )
    monkeypatch.setattr(builtins, "input", lambda _: next(user_inputs))
    scans = collect_scan_choices()
...
def test_scan_targets_creates_json(tmp_path, monkeypatch):
    # Prepare config
    monkeypatch.setattr("fnw.config", {
    ...
    def dummy_scan(ip, mode):
        return {"ip": ip, "type": "tcp", "output": "", "file": str(tmp_path / f"{ip}.txt")}
    ...
    assert json_file.exists() or any(f.endswith(".json") for f in os.listdir(tmp_path))
...
def test_discovery_creates_targets_file(tmp_path):
    ...
    assert all(l.startswith("192.168.0.") for l in lines)
...
def test_scan_targets_no_targets_file(tmp_path, capsys):
    monkeypatch = pytest.MonkeyPatch()
    monkeypatch.setattr(fnw, "config", {"output_directory": str(tmp_path), "thread_count": 1, "enable_json": False})
